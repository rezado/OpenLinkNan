...
class DSECtrlUnitImp(wrapper: DSECtrlUnit)(implicit p: Parameters) extends LazyRawModuleImp(wrapper) with HasXSParameter {
  ...
  val io = IO(new Bundle{
    ...
    val apply_cfg = Input(Bool())
    val ctrlSel = Output(UInt(8.W))
    val maxInstrCnt = Output(UInt(64.W))
    val epoch = Output(UInt(64.W))
    val maxEpoch = Output(UInt(64.W))
  })
...
//    val dcacheMSHRs = Wire(UInt(64.W))
    ...
    io.maxEpoch := max_epoch
    io.epoch := epoch
    io.maxInstrCnt := max_instr_cnt
    io.ctrlSel := ctrlSel
